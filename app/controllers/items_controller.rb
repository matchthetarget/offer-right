class ItemsController < ApplicationController
  ...
  def index
    ...
    @items = @q.result(:distinct => true).includes(:seller, :buyer, :messages, :category).page(params[:page]).per(10)
  end
  ...
  def current_user_must_be_item_seller
    ...
    def item_params
      params.require(:item).permit(:title, :price, :seller_id, :description, :category_id, :location, :buyer_id, :image)
    end
end
